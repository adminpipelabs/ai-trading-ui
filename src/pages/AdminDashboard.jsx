    ...
    <div className="flex min-h-screen" style={{ background: theme.bgSecondary, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2" style={{ color: theme.textPrimary }}>Admin Dashboard</h1>
            <p className="text-sm" style={{ color: theme.textMuted }}>Welcome back, {user?.name || user?.wallet_address || 'Admin'}</p>
          </div>
          ...
          {/* Chat Interface */}
          <div className="rounded-xl" style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, padding: '24px' }}>
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: theme.accentLight, color: theme.accent }}>
                <Bot size={20} />
              </div>
              <div>
                <h2 className="text-lg font-semibold" style={{ color: theme.textPrimary }}>AI Assistant</h2>
                <p className="text-xs" style={{ color: theme.textMuted }}>Ask me anything about your platform</p>
              </div>
            </div>

            <div className="space-y-4 mb-6" style={{ maxHeight: '400px', overflowY: 'auto' }}>
              {messages.map((msg, i) => (
                <Message key={i} message={msg} theme={theme} isDark={isDark} />
              ))}
              {isLoading && (
                <div className="flex gap-3 mb-5">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: theme.accentLight, color: theme.accent }}>
                    <Bot size={18} />
                  </div>
                  <div className="flex-1 p-4 rounded-2xl text-sm" style={{ background: theme.bgSecondary, color: theme.textMuted }}>
                    Thinking...
                  </div>
                </div>
              )}
